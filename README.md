...
## 4. 终端复现指南 & 零成本本地沙盒 (100% Reproducibility)

**🔥 核心突破：Aegis Shadow-Node 双模架构**
我们深知，要求所有开发者和评委去租用毫秒级企业 RPC 节点来进行攻防复现是不现实的。为了彻底击碎物理网络的复现壁垒，本项目独创了 **`PROD` (生产级主网监听) + `LOCAL_MOCK` (本地暗影沙盒)** 双模架构。

我们在底层解耦了网络层，并内置了一个轻量级的 WSS 内存池模拟器 (`shadow_mempool_mock.py`)。评委只需在普通笔记本上运行该模拟器，即可在本地构建一个与 X Layer 主网完全一致的虚拟 Mempool，**零成本、零延迟地体验 AI Agent 抓取黑客闪电贷、并在毫秒内完成 MEV 抢跑拦截的全套底层逻辑。**

```bash
# 1. 克隆底层核心仓库
git clone [https://github.com/YourName/Aegis-Matrix.git](https://github.com/YourName/Aegis-Matrix.git)
cd Aegis-Matrix
